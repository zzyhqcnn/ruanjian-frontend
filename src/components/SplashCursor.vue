...
let colorChangeSpeed = 0.005 // Increased for even faster color changes
...
onMounted(() => {
  let SIM_RESOLUTION = props.simResolution,
    ...
    COLOR_UPDATE_SPEED = props.colorChangeSpeed,
    CURL = props.curl,
  ...
  if (canvas) {
    ...
    // Initialize pointer with active state
    pointers[0].down = true // Always consider the pointer active
  ...
  `,
    ...
    function updateColors(dt: number) {
      hue = (hue + COLOR_UPDATE_SPEED) % 1.0
      currentColor = HSVtoRGB(hue, 1.0, 1.0)
      currentColor.r *= props.colorIntensity
      currentColor.g *= props.colorIntensity
      currentColor.b *= props.colorIntensity
    ...
    function clickSplat(pointer: Pointer) {
      // Use current color for click splats too
      const color = { ...currentColor }
      // Make the click effect more prominent
      color.r *= 3
      color.g *= 3
      color.b *= 3
    ...
    // Improved coordinate calculation for better sync
    function getRelativeMousePosition(e: MouseEvent): { posX: number; posY: number } {
    ...
    function getRelativeTouchPosition(touch: Touch): { posX: number; posY: number } {
      // Get the position relative to the viewport
    ...
    function updatePointerMoveData(pointer: Pointer, posX: number, posY: number) {
      ...
      pointer.color = currentColor // Always use current color
    }
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      // Always update pointer position
      updatePointerMoveData(pointer, posX, posY)

      // Always create splats on movement - no need to check if moved
      const smallSplat = { ...currentColor }
      splat(
        pointer.texcoordX,
        pointer.texcoordY,
        pointer.deltaX * config.SPLAT_FORCE * 0.7, // Increased effect strength
        pointer.deltaY * config.SPLAT_FORCE * 0.7,
        smallSplat,
      )
    }

    // We don't need mouse up/down events since the pointer is always active

    // Add mouse tracking
    window.addEventListener('mousemove', onMouseMove)

    // Touch events - simplified to just respond to movement
    const onTouchMove = (e: TouchEvent) => {
      // 移除 preventDefault() 以允许事件冒泡到下层元素
      const touches = e.targetTouches
      const pointer = pointers[0]
      if (touches.length > 0) {
        const { posX, posY } = getRelativeTouchPosition(touches[0])
        updatePointerMoveData(pointer, posX, posY)

        // Always create splat effect on touch move
        const smallSplat = { ...currentColor }
        splat(
          pointer.texcoordX,
          pointer.texcoordY,
          pointer.deltaX * config.SPLAT_FORCE * 0.7,
          pointer.deltaY * config.SPLAT_FORCE * 0.7,
          smallSplat,
        )
      }
